...
for i in {1..3}; do
    ...
    TEAM_ID=$(echo "$TEAM_OUTPUT" | grep "Team created:" | awk '{print $3}')
    ...
    DEVICE_ID=$(echo "$DEVICE_OUTPUT" | grep "Device Info for" | awk '{print $4}')
    IDENTITY_KEY=$(echo "$DEVICE_OUTPUT" | grep "Identity Key:" | sed 's/.*Identity Key: //')
    SIGNING_KEY=$(echo "$DEVICE_OUTPUT" | grep "Signing Key:" | sed 's/.*Signing Key: //')
    ENCODING_KEY=$(echo "$DEVICE_OUTPUT" | grep "Encoding Key:" | sed 's/.*Encoding Key: //')
...
# Sync to ensure role assignments propagate
echo "Syncing to ensure role assignments propagate..."
aranya --uds-path /tmp/aranya1/run/uds.sock sync-now $MAIN_TEAM_ID
sleep 2

# Network IDs will be assigned later by the Operator daemon after team setup
echo "=== Network IDs will be assigned by Operator daemon ==="
...
# Ensure Operator daemon has the main team as active storage
echo "Adding main team to Operator daemon..."
# Get the seed from daemon 1's team info
TEAM_INFO=$(aranya --uds-path /tmp/aranya1/run/uds.sock device-info $MAIN_TEAM_ID)
SEED_IKM=$(echo "$TEAM_INFO" | grep "Seed IKM:" | sed 's/.*Seed IKM: //')
echo "Using seed IKM: $SEED_IKM"
aranya --uds-path /tmp/aranya3/run/uds.sock add-team $MAIN_TEAM_ID "$SEED_IKM" || echo "Team may already exist on Operator daemon"

# Wait for team to be active on Operator daemon
sleep 3

# Add devices to the team on Operator daemon (needed for network ID assignment)
echo "Adding devices to team on Operator daemon..."
aranya --uds-path /tmp/aranya3/run/uds.sock add-device $MAIN_TEAM_ID ${IDENTITY_KEYS[1]} ${SIGNING_KEYS[1]} ${ENCODING_KEYS[1]} || echo "Device 1 may already exist"
aranya --uds-path /tmp/aranya3/run/uds.sock add-device $MAIN_TEAM_ID ${IDENTITY_KEYS[2]} ${SIGNING_KEYS[2]} ${ENCODING_KEYS[2]} || echo "Device 2 may already exist"

# Wait for devices to be added
sleep 3

# Sync to ensure state propagates
echo "Syncing to ensure state propagation..."
aranya --uds-path /tmp/aranya3/run/uds.sock sync-now $MAIN_TEAM_ID
sleep 2

# Verify devices are in team storage on Operator daemon
echo "Verifying devices in team storage on Operator daemon..."
aranya --uds-path /tmp/aranya3/run/uds.sock list-devices $MAIN_TEAM_ID

# Assign network IDs to devices for AQC communication (from Operator daemon)
echo "Assigning network ID to device 1: $DEVICE_1_ID"
if aranya --uds-path /tmp/aranya3/run/uds.sock assign-aqc-net-id $MAIN_TEAM_ID $DEVICE_1_ID "127.0.0.1:6001"; then
    echo "Network ID assigned successfully to device 1"
else
    echo "ERROR: Failed to assign network ID to device 1"
fi

echo "Assigning network ID to device 2: $DEVICE_2_ID"
if aranya --uds-path /tmp/aranya3/run/uds.sock assign-aqc-net-id $MAIN_TEAM_ID $DEVICE_2_ID "127.0.0.1:6002"; then
    echo "Network ID assigned successfully to device 2"
else
    echo "ERROR: Failed to assign network ID to device 2"
fi
...
# List label assignments for each device
for device_id in "${REAL_DEVICE_IDS[@]}"; do
    echo "Label assignments for device $device_id:"
    aranya --uds-path /tmp/aranya1/run/uds.sock -v list-label-assignments $MAIN_TEAM_ID $device_id
    echo ""
done
...
if [ -n "$SENSOR_LABEL_ID" ] && [ -n "$CONTROL_LABEL_ID" ]; then
    ...
    (aranya --uds-path /tmp/aranya1/run/uds.sock \
        listen-data --timeout 10 $MAIN_TEAM_ID $DEVICE_1_ID $SENSOR_LABEL_ID) &
    LISTENER_PID=$!
    ...
    echo "Command: send-data $MAIN_TEAM_ID $DEVICE_2_ID $SENSOR_LABEL_ID 'Temperature: 23.5°C, Humidity: 65%'"
    aranya --uds-path /tmp/aranya2/run/uds.sock \
        send-data $MAIN_TEAM_ID $DEVICE_2_ID $SENSOR_LABEL_ID "Temperature: 23.5°C, Humidity: 65%"
    ...
    # Extract PSKs for this channel (using show-channels output)
    SENSOR_PSK_IDENTITIES=($(echo "$SENSOR_CHANNELS_OUTPUT" | grep -oE "[a-f0-9]{64,68}" || true))
    SENSOR_PSK_SECRETS=($(echo "$SENSOR_CHANNELS_OUTPUT" | grep -oE "[a-f0-9]{64}" || true))
    ...
    aranya --uds-path /tmp/aranya1/run/uds.sock show-channels $MAIN_TEAM_ID > /tmp/psks_run${RUN_NUM}.txt 2>&1
    ...
    # Start listener in background
    (aranya --uds-path /tmp/aranya1/run/uds.sock \
        listen-data --timeout 10 $MAIN_TEAM_ID $DEVICE_1_ID $CONTROL_LABEL_ID) &
    LISTENER_PID=$!
    ...
    echo "Command: send-data $MAIN_TEAM_ID $DEVICE_2_ID $CONTROL_LABEL_ID 'SET_MODE:STANDBY'"
    aranya --uds-path /tmp/aranya2/run/uds.sock \
        send-data $MAIN_TEAM_ID $DEVICE_2_ID $CONTROL_LABEL_ID "SET_MODE:STANDBY"
    ...
    # Extract PSKs for this channel (using show-channels output)
    CONTROL_PSK_IDENTITIES=($(echo "$CONTROL_CHANNELS_OUTPUT" | grep -oE "[a-f0-9]{64,68}" || true))
    CONTROL_PSK_SECRETS=($(echo "$CONTROL_CHANNELS_OUTPUT" | grep -oE "[a-f0-9]{64}" || true))
    ...
    aranya --uds-path /tmp/aranya2/run/uds.sock \
        send-data $MAIN_TEAM_ID $DEVICE_2_ID $SENSOR_LABEL_ID "Temperature: 25.8°C, Humidity: 58% (2nd reading)"
    ...
    # Extract PSKs for rotated channel (using show-channels output)
    ROTATED_PSK_IDENTITIES=($(echo "$ROTATED_CHANNELS_OUTPUT" | grep -oE "[a-f0-9]{64,68}" || true))
    ROTATED_PSK_SECRETS=($(echo "$ROTATED_CHANNELS_OUTPUT" | grep -oE "[a-f0-9]{64}" || true))
...
if [ ${#CHANNEL_IDS[@]} -gt 0 ]; then
    ...
    CHANNEL_PSK_OUTPUT=$(aranya --uds-path /tmp/aranya1/run/uds.sock show-channels $MAIN_TEAM_ID)
...
ALL_PSKS_OUTPUT=$(aranya --uds-path /tmp/aranya1/run/uds.sock show-channels $MAIN_TEAM_ID)
